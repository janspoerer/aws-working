...
<div class="db-header clearfix">
    ...
    <div class="db-header-extra"> <button data-toggle="modal" data-target="#addObjectiveModal" class="btn btn-primary btn-xs"><?php echo lang("ctn_853") ?></button> </div>
</div>
...
<div class="db-header clearfix">
    ...
    <div class="db-header-extra"> <button id="updatedetails-button-update" type="button" class="btn btn-default btn-xs"> <span class="glyphicon glyphicon-refresh spin"></span></button> <button onclick="update_task()" class="btn btn-primary btn-xs"><?php echo lang("ctn_864") ?></button> </div>
</div>
...
<div class="db-header clearfix">
    ...
    <div class="db-header-extra"> <button data-toggle="modal" data-target="#addMemberModal" class="btn btn-primary btn-xs"><?php echo lang("ctn_868") ?></button> </div>
</div>

<table class="table table-bordered table-hover">
<tr class="table-header"><td width="40"><?php echo lang("ctn_869") ?></td><td><?php echo lang("ctn_52") ?></td></tr>
<?php foreach($task_members->result() as $r) : ?>
<tr><td> <?php echo $this->common->get_user_display(array("username" => $r->username, "avatar" => $r->avatar, "online_timestamp" => $r->online_timestamp)) ?></td><td><button id="remind-user-<?php echo $r->ID ?>" class="btn btn-warning btn-xs" onclick="remind_user(<?php echo $r->ID ?>)" title="<?php echo lang("ctn_870") ?>"><span class="glyphicon glyphicon-send"></span></button> <a href="<?php echo site_url("tasks/remove_member/" . $r->userid . "/" . $task->ID . "/" . $this->security->get_csrf_hash()) ?>" class="btn btn-danger btn-xs"><?php echo lang("ctn_871") ?></a></td></tr>
<?php endforeach; ?>
</table>
</div>
