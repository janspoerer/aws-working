...
function build_chat_area(data) 
{
	...
	$('#active_chat_bubble_' + data.chatid).append('<div class="chat-top-bar">'+data.title+' <div class="pull-right"><a href="'+global_base_url +'chat/view/' +data.chatid +'" class="white-icon"><span class="glyphicon glyphicon-cog chat-icon"></span></a> <span class="glyphicon glyphicon-trash click chat-icon" onclick="delete_chat_window('+data.chatid+')"></span> <span class="glyphicon glyphicon-minus click chat-icon" onclick="close_active_chat_window('+data.chatid+')"></span></div></div>');
	$('#active_chat_bubble_' + data.chatid).append('<div class="chat-chat-body" id="active_chat_window_'+data.chatid+'"></div>');
	$('#active_chat_bubble_' + data.chatid).append('<div class="chat-main-reply"><input type="text" name="reply" class="form-control" id="chat_input_message_'+data.chatid+'" placeholder="Send message ..." onkeypress="return wait_for_enter(event, '+data.chatid+');"></div>');
					...
					if(!$('#active_chat_bubble_' + chat.chatid).length) {
						...
						if($('#active_chat_window_' + chat.chatid).length) {
							...
							if(chat.unread == 1) {
								...
								$('#active_chat_bubble_' + chat.chatid).html('<span class="badge-chat small-text">NEW</span>' + chat.title);
							} else {
	...
	$.ajax({
			...
			success: function(data) {
				if($('#active_chat_window_' + id).length) {
					...
					// Update active chat bubbles
					if(data.unread == 1) {
						$('#active_chat_bubble_' + data.chatid).html('<span class="badge-chat small-text">NEW</span>' + data.title);
					} else {
