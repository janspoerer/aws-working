	...
	public function __construct() 
	{
		...
		$this->oauth_secret = $CI->input->cookie($config . "oauthsecret", TRUE);
 		...
 		$select = "users.`ID`, users.`username`, users.`email`, 
				...
				users.paypal_email, users.address_1, users.address_2, 
				users.city, users.state, users.zipcode, users.country,
				users.active, users.activate_code,
				users.stripe_secret_key, users.stripe_publish_key,
				users.checkout2_accountno, users.checkout2_secret,
				...
				user_roles.quote_manage, user_roles.ID as user_role_id,
