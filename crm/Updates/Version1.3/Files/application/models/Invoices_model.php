...
class Invoices_Model extends CI_Model 
{
	...
	public function get_invoice($id) 
	{
		return $this->db->where("invoices.ID", $id)
			...
			invoices.paypal_email, invoices.first_name, invoices.last_name,
			invoices.address_1, invoices.address_2, invoices.city, 
			invoices.state, invoices.zipcode, invoices.country, invoices.paid_by,
			invoices.template, invoices.stripe, invoices.checkout2, invoices.paypal,
			invoices.guest_name, invoices.guest_email,
			...
			u2.stripe_secret_key, u2.stripe_publish_key, u2.checkout2_accountno,
			u2.checkout2_secret, u2.paypal_email as user_paypal_email,
			projects.name as projectname,
			currencies.name as currencyname, currencies.symbol,
			currencies.code")
			->join("users", "users.ID = invoices.clientid", "left outer")
