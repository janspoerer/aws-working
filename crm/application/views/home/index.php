...
<div class="col-md-9">
	...
	<div class="block-area align-center">
		<h4 class="home-label">Project Overview</h4>
    ...
    $str .= $dtF->diff($dtT)->format('%a days, %h hours'); // %m minitues, %s seconds
    return $str;
}

	$prj_ids = array();
	$prjs = $projects_by_due_date->result();
			

		foreach($prjs as $prj): ?>

			<?php
			// ID, complete, complete_sync
			//print_r($prj);
			// get tasks from project
			//$tasks = $task_model->get_all_tasks_for_project($prj->ID);
			$tasks_limit = 5;
			$prj_tasks = $task_model->get_all_project_tasks_by_due_date($prj->ID, 100, $tasks_limit)->result();
			
			foreach($prj_tasks as $i => $t): 
				// ID, name, due_date, complete,
				$due_time = $t->due_date - time();
			?>
					<tr>
						<?php if($i == 0): ?>
						<td width="8%" rowspan="<?php echo $tasks_limit ?>">
							 <canvas id="chart-area-project<?php echo $prj->ID ?>" />
						</td>
						<td width="12%" class="text-left" rowspan="<?php echo $tasks_limit ?>">
							<strong>
								<a href="<?php echo site_url("projects/view/" . $prj->ID) ?>"><?php echo $prj->name ?></a>
							</strong>
							<!--(<?php echo timeTo($prj->due_date - time()); ?>)-->
						</td>
						<?php endif; ?>
						<td class="text-left"><a href="<?php echo site_url("tasks/view/" . $t->ID) ?>"><?php echo $t->name ?></a></td>
						<td><?php echo timeTo($due_time); ?></td>
						<td>
							<div class="progress" style="margin-bottom: 0px !important;">
							  <div class="progress-bar progress-bar-success progress-bar-striped" role="progressbar" aria-valuenow="<?php echo $t->complete ?>" aria-valuemin="0" aria-valuemax="100" style="width: <?php echo $t->complete ?>%;" title="<?php echo $t->complete ?>%" data-toggle="tooltip" data-placement="bottom">
							    <span class="sr-only"><?php echo $t->complete ?>%</span>
							  </div>
							</div>
						</td>
					</tr>
					
			<?php endforeach; // End tasks
			?>
			
			<?php for($i=0; $i < $tasks_limit - count($prj_tasks); $i++): ?>
...
var myPieChart = new Chart($("#projectTypesChart"),{
	...
	$(document).ready(function() {
  	...
  	new Chart(document.getElementById("chart-area-project<?php echo $prj->ID ?>").getContext("2d"), config<?php echo $prj->ID ?>);
		<?php endforeach; ?>
		/////////////////////////////////////
	});
</script>