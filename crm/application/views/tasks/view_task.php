...
<div class="db-header clearfix">
    ...
    <div class="db-header-extra"> <?php if($this->common->has_permissions(array("admin", "project_admin", "task_manage", "task_worker"), $this->user)) : ?><button id="status-button-update" type="button" class="btn btn-default btn-xs"> <span class="glyphicon glyphicon-refresh spin"></span></button>
      <div class="btn-group">
        <div class="btn-group">
        <?php if(isset($form_info->formid) && isset($form_info->hash)) : ?>
          <a href="<?php echo site_url("leads/view_form_full/".$form_info->formid."/".$form_info->hash) ?>" class="btn btn-primary btn-xs"><?php echo lang("ctn_2005") ?></a>
        <?php else : ?>
          <a href="#" class="btn btn-danger btn-xs"><?php echo lang("ctn_2006") ?></a>
        <?php endif; ?>
      </div>
      </div>
    <?php endif; ?>
...
<?php echo $task->description ?>
   ...
   <div class="btn-group">
      ...
      <a href="<?php echo site_url("tasks/edit_task/" . $task->ID) ?>" class="btn btn-warning btn-xs"><?php echo lang("ctn_851") ?></a> <a href="<?php echo site_url("tasks/delete_task/" . $task->ID . "/" . $this->security->get_csrf_hash()) ?>" class="btn btn-danger btn-xs" onclick="return confirm('<?php echo lang("ctn_508") ?>')"><?php echo lang("ctn_850") ?></a>
      </div>
</div>
...
<div class="db-header clearfix">
    ...
    <div class="db-header-extra"> 
      <?php if($this->common->has_permissions(array("admin", "project_admin", "task_manage", "task_worker"), $this->user)) : ?>
         <a href="<?php echo site_url("leads/edit_lead/".$leadid) ?>" class="btn btn-warning btn-xs"><?php echo lang("ctn_2009") ?></a>
      <?php endif; ?> 
    </div>
</div>

<?php foreach($objectives->result() as $r) : 
  // Q/A updated by Malte <pre><?php print_r($r) </pre>
?>

	<div class="row" data-leadid="000">
	  <div class="col-md-12">
			<div class="pull-right">
         <?php if($r->complete): ?><a href="<?php echo site_url("leads/edit_lead/".$leadid) ?>" class="btn btn-success btn-xs"><?php echo lang("ctn_2010") ?></a><?php endif; ?>
         <?php if(!$r->complete): ?><a href="<?php echo site_url("leads/edit_lead/".$leadid) ?>" class="btn btn-warning btn-xs"><?php echo lang("ctn_2011") ?></a><?php endif; ?>
			</div>
		
	    <strong><?php echo $r->title ?></strong>:<br><span class="label label-default"><?php echo $r->answer ?></span>
	  	
      <p class="small-text"><?php echo $r->description ?></p>
			  <?php //if($r->complete) echo"<span class='completeText'> <span class='glyphicon glyphicon-ok'></span> ".lang("ctn_855")."</span>" ?>
</div>
