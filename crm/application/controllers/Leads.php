...
class Leads extends CI_Controller 
{
	...
	public function lead_page($page) 
	{
		...
		foreach($leads->result() as $r) {
			...
			// Added by Clemens, 14.10.2017
			$bank = "";
			$year = "";
			if(!isset($r->taskid) || $r->taskid == 0) {
				$bank = lang("ctn_2001");
				$year = lang("ctn_2003");
			} else { 
				$data = $this->leads_model->get_project_data($r->taskid);
				$bank = $data[0];
				$year = $data[1];
			}
	...
	public function view_form_full($id,$hash) 
	{
		...
		// Added by Clemens, 14.10.207
		// Get the project data for the given hash and check if the correct form
		// is associated with this hash code
		$project_data = $this->leads_model->get_project_info_for_hash($hash);
		if(isset($project_data) && $project_data->formid != $id) {
			$project_data = NULL;
		}
	...
	public function process_form($id, $guest=0) 
	{
		...
		$hash = $this->common->nohtml($this->input->post("hash"));
		$project_info = $this->leads_model->get_project_info_for_hash($hash);
		...
		if(isset($project_info->taskid)) $taskid = $project_info->taskid;
		else $taskid = 0;
		...
		if(!$guest) { 
			...
			redirect(site_url("leads/view_form_full/" . $id . "/". $hash));
		}
	}
	...
	public function trigger_task_objective_update() {
