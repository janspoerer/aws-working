...
<div class="active_chat_bubble" onclick="load_active_chat(<?php echo $r->ID ?>)" id="active_chat_bubble_<?php echo $r->ID ?>">
	<?php if($r->unread) : ?><span class="badge-chat small-text"><?php echo lang("ctn_1310") ?></span><?php endif; ?> <?php echo $r->title ?> 
</div>
<?php else : ?>
	<?php 
		$limit = 5;
...
<div class="chat-top-bar"><?php echo $r->title ?> <div class="pull-right"><a href="<?php echo site_url("chat/view/" . $r->ID) ?>" class="white-icon"><span class="glyphicon glyphicon-cog chat-icon"></span></a> <span class="glyphicon glyphicon-trash click chat-icon" onclick="delete_chat_window(<?php echo $r->ID ?>)"></span> <span class="glyphicon glyphicon-minus click chat-icon" onclick="close_active_chat_window(<?php echo $r->ID ?>)"></span></div></div>
...
<div class="chat-main-reply">
	<input type="text" name="reply" class="form-control" id="chat_input_message_<?php echo $r->ID ?>" placeholder="<?php echo lang("ctn_1311") ?>" onkeypress="return wait_for_enter(event, <?php echo $r->ID ?>);">
</div>
</div>
<?php endif; ?>
<?php endforeach; ?>
