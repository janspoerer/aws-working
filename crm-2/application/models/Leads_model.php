...
class Leads_Model extends CI_Model 
{
  ...
  // Added by Clemens, 14.10.2017
	public function get_project_data($taskid) {
		
		$res = $this->db->where("ID", $taskid)
		->select("project_tasks.name as project_task_name, 
							project_tasks.projectid as project_task_project_id")
		->get("project_tasks");

		$r = $res->row();
	if(!isset($r->project_task_name)) {
			$task_name = "None";
		} else { 
			$task_name = $r->project_task_name;
		}
		
		$project_name = "";
		if(isset($r->project_task_project_id)) {
			$res = $this->db->where("ID", $r->project_task_project_id)
												->select("projects.name as project_name")
												->get("projects");
			$project_name = $res->row()->project_name;
		}
		
		return [$task_name,$project_name];
	}
 ...
 // Modified by Clemens, 14.10.2017
	...
	public function get_project_info_for_hash($hash) {
		$res = $this->db
		->where("hash", $hash)
		->select("taskid, formid, hash")
