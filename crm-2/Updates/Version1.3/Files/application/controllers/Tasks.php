...
class Tasks extends CI_Controller 
{
	...
	public function tasks_page($page = "index", $projectid=0, $u_status =0) 
	{
		...
		foreach($tasks->result() as $r) {
			...
			if($page == "client") {
				$options = '<a href="'.site_url("tasks/view/" . $r->ID) .'" class="btn btn-info btn-xs">'.lang("ctn_886").'</a>';
			} else {
				$options = '<a href="'.site_url("tasks/view/" . $r->ID) .'" class="btn btn-info btn-xs">'.lang("ctn_886").'</a> <a href="'.site_url("tasks/edit_task/" . $r->ID) .'" class="btn btn-warning btn-xs" title="'.lang("ctn_55").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-cog"></span></a> <a href="'.site_url("tasks/delete_task/" . $r->ID . "/" . $this->security->get_csrf_hash()).'" class="btn btn-danger btn-xs" onclick="return confirm(\''.lang("ctn_508").'\')" title="'.lang("ctn_57").'" data-toggle="tooltip" data-placement="bottom"><span class="glyphicon glyphicon-trash"></span></a>';
			}
